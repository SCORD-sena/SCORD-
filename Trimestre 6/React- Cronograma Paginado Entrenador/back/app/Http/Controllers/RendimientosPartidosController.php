...
class RendimientosPartidosController extends Controller
{
    ...
    /**
     * Validar que el jugador autenticado solo pueda ver sus propias estadísticas
     */
    private function validarAccesoJugador($idJugadores)
    {
        $persona = auth()->user();
        
        // Si es jugador (rol 3), solo puede ver SUS propias estadísticas
        if ($persona->idRoles === 3) {
            $jugador = Jugadores::where('idPersonas', $persona->idPersonas)->first();
            
            if (!$jugador || $jugador->idJugadores != $idJugadores) {
                return false;
            }
        }
    ...
    public function getTotalStatsByPlayer($idJugadores)
    {
        try {
            // Validar acceso
            if (!$this->validarAccesoJugador($idJugadores)) {
                return response()->json([
                    'success' => false,
    ...
    /**
     * Obtener últimos partidos de un jugador con sus estadísticas
     */
