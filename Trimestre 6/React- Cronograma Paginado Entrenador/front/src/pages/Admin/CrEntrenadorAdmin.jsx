...
// Configuración de la API (sin cambios)
...
const CrEntrenadorAdmin = () => {
  const ITEMS_PER_PAGE = 5; // Límite de registros por página
  ...
  // --- Funciones de Fetch (sin cambios significativos) ---

  const fetchCronogramas = async () => {
    try {
      const res = await api.get("/cronogramas");
      setCronogramas(res.data);
      // Asume que solo los cronogramas de tipo "Entrenamiento" van al estado 'entrenamientos'
  ...
  // --- Lógica de Manejo de Formularios y CRUD (Sin cambios) ---

  const handleAgregarEntrenamiento = async (e) => {
    // ... Lógica de agregar entrenamiento ...
    e.preventDefault();
  ...
  const handleBorrar = async (id) => {
    // ... Lógica de borrar cronograma (entrenamiento o partido padre) ...
  ...
  const handleBorrarPartidoAPI = async (id) => {
    // ... Lógica de borrar partido (solo el registro de partido, que debería borrar el cronograma asociado en el backend) ...
    // NOTA: Se está llamando a `api.delete('/partidos/${id}')`. Si el backend no elimina el cronograma asociado, habrá que cambiar esta lógica a la de `handleBorrar(idCronograma)` después de obtener el idCronograma. Para mantenerlo simple, asumo que eliminar partido también elimina el cronograma asociado.
    
    // Si la API de partidos elimina el cronograma asociado, el fetchCronogramas lo actualizará.
    const result = await Swal.fire({
      title: "¿Estás seguro?",
      text: "Esta acción no se puede deshacer",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        await fetchPartidosAPI();
        await fetchCronogramas(); // Para actualizar la lista de entrenamientos/partidos
    ...
    <div>
      ...
      <main className="container my-5">
        ...
        <section className="mb-5">
          ...
          <div className="mb-3">
            <input
              ...
              onChange={(e) => {
                ...
                setCurrentPageEntrenamiento(1); // Resetear a la primera página al buscar
              }}
            />
          </div>
          ...
          {/* Paginación para Entrenamientos */}
          <Pagination
          ...
          <div className="mt-4">
            ...
            {loadingCategorias ? (
              ...
              >
                {/* ... Campos de formulario de entrenamiento (sin cambios) ... */}
        ...
        <section className="mb-5">
          ...
          {/* Búsqueda para Partidos */}
          <div className="mb-3">
            <input
              type="text"
              className="form-control"
              placeholder="Buscar partido por rival, fecha, ubicación, cancha, formación o categoría..."
              value={searchTermPartido}
              onChange={(e) => {
                setSearchTermPartido(e.target.value);
                setCurrentPagePartido(1); // Resetear a la primera página al buscar
              }}
            />
          </div>
          ...
          <div className="mt-4">
            ...
            {loadingCategorias ? (
              ...
              >
                {/* ... Campos de formulario de partido (sin cambios) ... */}
                <div className="col-md-6">
