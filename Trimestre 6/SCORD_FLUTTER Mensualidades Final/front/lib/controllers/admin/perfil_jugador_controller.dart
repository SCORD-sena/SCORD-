...
class PerfilJugadorController extends ChangeNotifier {
  ...
  Future<void> fetchInitialData() async {
    ...
    try {
      final results = await Future.wait([
        _jugadorService.fetchJugadores(),
  ...
  Future<void> seleccionarJugador(int? idJugador) async {
    ...
    try {
      jugadorSeleccionado = jugadores.firstWhere(
        (j) => j.idJugadores == idJugador,
      ...
      print('📡 Recargando datos completos del jugador ID: $idJugador');
      final jugadorCompleto = await _jugadorService.show(idJugador);
      
      jugadorSeleccionado = jugadorCompleto;
      
      print('✅ Datos completos cargados');
      print('   - fechaIngresoClub: ${jugadorSeleccionado?.fechaIngresoClub}');
      print('   - fechaVencimientoMensualidad: ${jugadorSeleccionado?.fechaVencimientoMensualidad}');
      print('   - estadoPago: ${jugadorSeleccionado?.estadoPago}');
      
      notifyListeners();
    } catch (e) {
      print('❌ Error al seleccionar jugador: $e');
      rethrow;
    }
  }
  ...
  List<String> validateForm() {
    ...
    if (controllers['numeroDocumento']!.text.isEmpty) {
      errors.add('El numero de documento es obligatorio.');
    }
  ...
  // ====== METODO CORREGIDO: Registrar pago de mensualidad ======
  Future<void> registrarPagoMensualidad() async {
    if (jugadorSeleccionado == null) {
      throw Exception('No hay jugador seleccionado');
    }

    try {
      print('💰 Registrando pago para jugador ID: ${jugadorSeleccionado!.idJugadores}');
      
      // Registrar el pago
      await _jugadorService.registrarPagoMensualidad(
        jugadorSeleccionado!.idJugadores,
      );

      print('✅ Pago registrado en el backend');

      // ====== RECARGAR DATOS COMPLETOS DEL JUGADOR ======
      print('🔄 Recargando datos actualizados...');
      final jugadorCompleto = await _jugadorService.show(jugadorSeleccionado!.idJugadores);
      
      // Actualizar el jugador seleccionado con los nuevos datos
      jugadorSeleccionado = jugadorCompleto;
      
      print('📅 Nueva fecha vencimiento: ${jugadorCompleto.fechaVencimientoMensualidad}');
      print('📊 Nuevo estado pago: ${jugadorCompleto.estadoPago}');
      
      // Actualizar la lista de jugadores
      final index = jugadores.indexWhere(
        (j) => j.idJugadores == jugadorCompleto.idJugadores,
      );
      
      if (index != -1) {
        jugadores[index] = jugadorCompleto;
      }

      // Notificar cambios a la UI
      notifyListeners();
      
      print('✅ Vista actualizada con nuevos datos de mensualidad');
    } catch (e) {
