import 'dart:convert'; 
import 'package:flutiii/models/competencia_model.dart';
...
class CronogramaService { 
  ...
  try { 
    ...
    if (response.statusCode == 200) { 
      print('📦 Response body: ${response.body}'); // ← VER QUÉ LLEGA
      final List data = json.decode(response.body); 
      
      // Ver cada item individualmente
      for (var item in data) {
        print('🔍 Item: $item');
        print('   idCategorias: ${item['idCategorias']} (${item['idCategorias'].runtimeType})');
      }
    ...
    print('❌ Error completo: $e'); // ← VER ERROR COMPLETO
 ...
 Future<List<Categoria>> getCategorias() async { 
  try { 
    ...
    if (response.statusCode == 200) { 
      print('📦 CATEGORIAS Response: ${response.body}'); // ← AGREGAR
      final List data = json.decode(response.body); 
      
      for (var item in data) {
        print('🔍 Categoria: $item'); // ← AGREGAR
      }
    ...
    print('❌ Error en getCategorias: $e'); // ← AGREGAR
    throw Exception('Error de conexión: $e'); 
  } 
} 
