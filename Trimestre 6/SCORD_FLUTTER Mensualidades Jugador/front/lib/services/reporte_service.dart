...
class ReporteService {
  ...
  Future<Uint8List?> getReportePDFBytes(int idJugador) async {
    try {
      print('📄 Obteniendo bytes del reporte PDF para jugador: $idJugador');
      ...
      print('✅ Status al obtener bytes: ${response.statusCode}');
      
      if (response.statusCode == 200) {
        return response.bodyBytes;
      } else {
        print('❌ Error en respuesta (bytes): ${response.statusCode}');
        print('❌ Body (bytes): ${response.body}');
      ...
      print('❌ Error obteniendo bytes del reporte: $e');
      rethrow;
    }
  }
  ...
  Future<String?> generarReportePDF(int idJugador) async {
    ...
    try {
      print('📄 Generando reporte PDF para jugador: $idJugador (Guardado Nativo)');
