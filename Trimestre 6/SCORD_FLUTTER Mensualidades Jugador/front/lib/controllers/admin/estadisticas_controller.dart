...
import 'package:universal_html/html.dart' as html; // Para descarga en Web (Requiere pubspec.yaml)
...
// ========================================
  // FUNCIONALIDAD DE REPORTES PDF
  // ========================================

  /// Genera el reporte PDF del jugador seleccionado.
  /// 
  /// Si se ejecuta en la web, realiza la descarga directa. 
  /// En otras plataformas, delega el guardado a ReporteService.
  /// Retorna la ruta del archivo o un mensaje de éxito para la descarga web.
  Future<String?> generarReporteJugador() async {
    ...
    try {
      ...
      if (pdfBytes == null || pdfBytes.isEmpty) {
        // Manejar el caso de bytes nulos o vacíos
        throw Exception('No se pudo generar el contenido del reporte (bytes nulos o vacíos)');
      }

      if (kIsWeb) {
        // 2. Lógica para WEB: Descargar el archivo directamente en el navegador.
        // Nombre de archivo simplificado a 'reporte_jugador.pdf'
        final nombreArchivo = 'reporte_jugador.pdf'; 
        
        final blob = html.Blob([pdfBytes]);
        final url = html.Url.createObjectUrlFromBlob(blob);
        
        // Simular un clic en un enlace para forzar la descarga
        ...
        // 3. Lógica para PLATAFORMAS NATIVAS: Delegar el guardado a ReporteService.
        // Se llama a la función que usa dart:io y pide permisos.
