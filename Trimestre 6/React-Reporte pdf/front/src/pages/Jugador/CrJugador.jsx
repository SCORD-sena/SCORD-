...
const CrJugador = () => {
  ...
  const filteredEntrenamientos = useMemo(() => {
    ...
    return entrenamientos.filter((entrenamiento) => {
      const dia = obtenerDiaSemana(entrenamiento.FechaDeEventos).toLowerCase();
      const fecha = formatearFecha(entrenamiento.FechaDeEventos).toLowerCase();
      
      return (
        dia.includes(lowerCaseSearch) ||
        fecha.includes(lowerCaseSearch) ||
        entrenamiento.Ubicacion?.toLowerCase().includes(lowerCaseSearch) ||
        entrenamiento.SedeEntrenamiento?.toLowerCase().includes(lowerCaseSearch) ||
        entrenamiento.Descripcion?.toLowerCase().includes(lowerCaseSearch)
      );
    });
  }, [entrenamientos, searchTermEntrenamiento]);
  ...
  const filteredPartidos = useMemo(() => {
    if (!searchTermPartido) return partidos;
    const lowerCaseSearch = searchTermPartido.toLowerCase();

    return partidos.filter((partido) => {
      const fecha = formatearFecha(partido.FechaDeEventos).toLowerCase();
      
      return (
        fecha.includes(lowerCaseSearch) ||
        partido.EquipoRival?.toLowerCase().includes(lowerCaseSearch) ||
        partido.Formacion?.toLowerCase().includes(lowerCaseSearch) ||
        partido.Ubicacion?.toLowerCase().includes(lowerCaseSearch) ||
        partido.CanchaPartido?.toString().includes(lowerCaseSearch)
      );
    });
  }, [partidos, searchTermPartido]);
  ...
  // ============================================================
  // COMPONENTE DE PAGINACIÓN REUTILIZABLE (IGUAL AL ADMIN)
  // ============================================================
              ...
              <section className="mb-5">
                ...
                <div className="mb-3">
                  <input
                    ...
                    placeholder="Buscar entrenamiento por día, fecha, ubicación, sede o descripción..."
                  ...
                  <table className="table table-bordered table-striped">
                    ...
                    <tbody>
                      {currentEntrenamientos.length > 0 ? (
                        currentEntrenamientos.map((entrenamiento) => (
                        ...
                        <tr>
                          <td colSpan="5" className="text-center">
                            {searchTermEntrenamiento 
                              ? "No se encontraron entrenamientos con esa búsqueda" 
              ...
              <section className="mb-5">
                ...
                <div className="mb-3">
                  <input
                    type="text"
                    className="form-control"
                    placeholder="Buscar partido por fecha, equipo rival, formación, ubicación o cancha..."
                    ...
                    <tbody>
                      {currentPartidos.length > 0 ? (
                        ...
                        <tr>
                          <td colSpan="5" className="text-center">
                            {searchTermPartido 
                              ? "No se encontraron partidos con esa búsqueda" 
