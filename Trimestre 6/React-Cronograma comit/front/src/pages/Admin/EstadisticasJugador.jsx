...
const guardarCambios = async () => {
  ...
  try {
    ...
    if (res.data.success && res.data.data) {
      ...
      // ✅ SOLUCIÓN: Recargar todo antes de salir del modo edición
  ...
  const eliminarEstadistica = async () => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        // OPCIÓN 1: Intentar con la ruta de últimos partidos
        try {
        ...
        // OPCIÓN 2: Si no funciona la ruta de últimos partidos, obtener todos y tomar el último
        if (!ultimoRegistro) {
          const todosRendimientos = await api.get(`/rendimientospartidos`);
          if (todosRendimientos.data && todosRendimientos.data.length > 0) {
            // Filtrar por el jugador seleccionado y tomar el último
            ...
            if (rendimientosJugador.length > 0) {
              // Ordenar por ID descendente y tomar el último (mayor ID)
  ...
  return (
    <div>
      ...
      {/* BOTONES DE ACCIÓN - SIGUIENDO EL MISMO PATRÓN */}
