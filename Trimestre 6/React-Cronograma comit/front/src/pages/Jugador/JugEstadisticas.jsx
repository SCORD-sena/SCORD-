...
const JugEstadisticas = () => {
  ...
  // ✅ SOLUCIÓN: Obtener solo MI perfil de jugador
  const obtenerMiPerfil = async () => {
    try {
      ...
      // Si es 404, significa que el usuario no tiene perfil de jugador
  ...
  const cargarEstadisticas = async () => {
    try {
      ...
      if (resEstadisticas.data.success) {
        ...
        // Si no hay estadísticas, mostrar valores en 0
        setEstadisticas({
          totales: {
            total_goles: 0,
            total_asistencias: 0,
      ...
      // Si es un error 404, significa que no hay estadísticas registradas
      if (err.response?.status === 404) {
        setEstadisticas({
          totales: {
            total_goles: 0,
