...
use App\Models\Cronogramas; // ⭐ ESTE YA LO TIENES
    ...
    public function getCompetenciasByCategoria($idCategorias)
{
    try {
        ...
        // 3. Combinar USANDO UNIQUE por idCompetencias para evitar duplicados
        $competencias = $competenciasUsadas
            ->merge($competenciasNuevas)
            ->unique('idCompetencias') // 🆕 ESTO ELIMINA DUPLICADOS
...
/**
 * Crear competencia Y asociarla con una categoría mediante cronograma
 */
public function storeWithCategoria(Request $request)
{
    $validator = Validator::make($request->all(), [
        'Nombre' => 'required|string|max:50',
        'TipoCompetencia' => 'required|string|max:30',
    ...
    try {
        ...
        Cronogramas::create([
            'FechaDeEventos' => now()->format('Y-m-d'),
            'Hora' => '00:00:00',
            'TipoDeEventos' => 'Competencia',
            'Descripcion' => 'Asociación: ' . $request->Nombre,
