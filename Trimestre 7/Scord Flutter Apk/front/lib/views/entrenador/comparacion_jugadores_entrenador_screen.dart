...
class ComparacionJugadoresEntrenadoresScreen extends StatefulWidget {
  const ComparacionJugadoresEntrenadoresScreen({Key? key}) : super(key: key);

  @override
  State<ComparacionJugadoresEntrenadoresScreen> createState() =>
      _ComparacionJugadoresScreenState();
}

class _ComparacionJugadoresScreenState
    extends State<ComparacionJugadoresEntrenadoresScreen> {
  ...
  void initState() {
    super.initState();
    _cargarCategoriasDelEntrenador();
  }

  Future<void> _cargarCategoriasDelEntrenador() async {
    setState(() {
...
if (persona == null) {
  throw Exception('No se pudo obtener información del usuario');
      ...
      final entrenador = await _entrenadorService.getEntrenadorByPersonaId(
        persona.idPersonas
      );

      if (entrenador == null) {
        throw Exception('No se encontró información del entrenador');
      }

      final categoriasAsignadas = entrenador.categorias ?? [];

      if (categoriasAsignadas.isEmpty) {
        setState(() {
          _categorias = [];
          _isLoadingCategorias = false;
          _errorMessage = 'No tienes categorías asignadas. Contacta al administrador.';
        });
        return;
      }

      setState(() {
        _categorias = categoriasAsignadas;
        _isLoadingCategorias = false;
      });

      debugPrint('✅ Categorías del entrenador cargadas: ${_categorias.length}');
      for (var cat in _categorias) {
        debugPrint('   - ${cat.descripcion}');
      }
      
    } catch (e) {
      debugPrint('❌ Error al cargar categorías del entrenador: $e');
      setState(() {
        _errorMessage = 'Error al cargar tus categorías asignadas';
        _isLoadingCategorias = false;
      });
    }
  }
...
Widget build(BuildContext context) {
  return Scaffold(
    ...
    drawer: EntrenadorDrawer(currentRoute: '/ComparacionJugadoresEntrenador'), // ✅ NUEVO
                            ...
                            const Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 4),
                                  Text(
                                    'Compare estadísticas de tus jugadores',
                                    style: TextStyle(
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                child: DropdownButtonFormField<Categoria>(
                                  ...
                                  hint: const Text('Seleccione una de tus categorías'),
                        ...
                        if (_categorias.isEmpty && !_isLoadingCategorias)
                          Container(
                            padding: const EdgeInsets.all(20),
                            decoration: BoxDecoration(
                          ...
                          Container(
                            ...
                            child: Row(
                              children: [
                                ...
                                Expanded(
                                  child: Text(
                                    'Seleccione una de tus categorías para comenzar',
                                    style: TextStyle(
                                      color: Colors.blue[900],
                                      fontSize: 14,
                                    ),
