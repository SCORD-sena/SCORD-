...
class _AgregarJugadorScreenState extends State<AgregarJugadorScreen> {
  ...
  Future<void> _crearJugador() async {
    ...
    try {
      ...
      // 5. Éxito
      await _showErrorDialog("¡Jugador creado!", "El jugador se creo correctamente.");

      // Limpiar formulario y navegar
      _resetForm();
      if (mounted) {
        await Navigator.of(context).pushReplacementNamed('/PerfilJugadorAdmin');
      }
      ...
      await _showErrorDialog("Error", errorMsg);
