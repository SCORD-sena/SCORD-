...
class RendimientosPartidosController extends Controller
{
    ...
    /**
 * NUEVO: Obtener rendimientos eliminados con información completa
 * (incluye jugador, partido, cronograma y competencia)
 */
public function getTrashed()
{
    try {
        // Primero solo traer los rendimientos sin relaciones
        $rendimientos = RendimientosPartidos::onlyTrashed()->get();
        
        \Log::info('Rendimientos encontrados: ' . $rendimientos->count());
        ...
        // Ahora intenta cargar las relaciones
        $rendimientos = RendimientosPartidos::onlyTrashed()
            ->with([
                'jugador.persona',
                'jugador.categoria',
        ...
        \Log::info('Con relaciones: ' . $rendimientos->count());

        // Formatear la respuesta...
        $rendimientosFormateados = $rendimientos->map(function ($rendimiento) {
            return [
                'IdRendimientos' => $rendimiento->IdRendimientos,
                'idPartidos' => $rendimiento->idPartidos,
                'idJugadores' => $rendimiento->idJugadores,
                'Goles' => $rendimiento->Goles,
                'GolesDeCabeza' => $rendimiento->GolesDeCabeza,
                'MinutosJugados' => $rendimiento->MinutosJugados,
                'Asistencias' => $rendimiento->Asistencias,
                'TirosApuerta' => $rendimiento->TirosApuerta,
                'TarjetasRojas' => $rendimiento->TarjetasRojas,
                'TarjetasAmarillas' => $rendimiento->TarjetasAmarillas,
                'FuerasDeLugar' => $rendimiento->FuerasDeLugar,
                'ArcoEnCero' => $rendimiento->ArcoEnCero,
                'deleted_at' => $rendimiento->deleted_at,
                
                'jugador' => [
                    'idJugadores' => $rendimiento->jugador->idJugadores,
                    'Dorsal' => $rendimiento->jugador->Dorsal,
                    'Posicion' => $rendimiento->jugador->Posicion,
                    'persona' => [
                        'idPersonas' => $rendimiento->jugador->persona->idPersonas,
                        'Nombre1' => $rendimiento->jugador->persona->Nombre1,
                        'Nombre2' => $rendimiento->jugador->persona->Nombre2,
                        'Apellido1' => $rendimiento->jugador->persona->Apellido1,
                        'Apellido2' => $rendimiento->jugador->persona->Apellido2,
                    ],
                    'categoria' => [
                        'idCategorias' => $rendimiento->jugador->categoria->idCategorias,
                        'Descripcion' => $rendimiento->jugador->categoria->Descripcion,
                    ]
                ],
                
                'partido' => [
                    'idPartidos' => $rendimiento->partido->idPartidos,
                    'EquipoRival' => $rendimiento->partido->EquipoRival,
                    'Formacion' => $rendimiento->partido->Formacion,
                    'cronograma' => [
                        'idCronogramas' => $rendimiento->partido->cronograma->idCronogramas,
                        'FechaDeEventos' => $rendimiento->partido->cronograma->FechaDeEventos,
                        'Ubicacion' => $rendimiento->partido->cronograma->Ubicacion,
                        'competencia' => $rendimiento->partido->cronograma->competencia ? [
                            'idCompetencias' => $rendimiento->partido->cronograma->competencia->idCompetencias,
                            'Nombre' => $rendimiento->partido->cronograma->competencia->Nombre,
                            'TipoCompetencia' => $rendimiento->partido->cronograma->competencia->TipoCompetencia,
                            'Ano' => $rendimiento->partido->cronograma->competencia->Ano,
                        ] : null
                    ]
                ]
            ];
        ...
        return response()->json([
            'success' => true,
            'data' => $rendimientosFormateados
        ], 200);

    } catch (\Exception $e) {
        \Log::error('Error en getTrashed: ' . $e->getMessage());
        \Log::error('Stack trace: ' . $e->getTraceAsString());
        
        return response()->json([
            'message' => 'Error al obtener rendimientos eliminados',
            'error' => $e->getMessage(),
            'line' => $e->getLine()
        ], 500);
    }
...
public function obtenerPorPartido($idJugadores, $idPartido){
    try {$rendimiento = RendimientosPartidos::where('idJugadores', $idJugadores)->where('idPartidos', $idPartido)->first();
