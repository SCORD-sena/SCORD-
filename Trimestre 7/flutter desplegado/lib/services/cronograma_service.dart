...
class CronogramaService { 
 ...
 Future<List<Categoria>> getCategorias() async { 
  ...
  // ============================================================
  // MÉTODOS PARA HISTORIAL DE CRONOGRAMAS
  // ============================================================

  /// Obtener cronogramas eliminados (papelera)
  Future<List<Cronograma>> fetchCronogramasEliminados() async {
    try {
      final headers = await _getHeaders();
      final response = await http.get(
        Uri.parse('$baseUrl/cronogramas/papelera/listar'),
        headers: headers,
      );
      ...
      if (response.statusCode == 200) {
        ...
        List<dynamic> cronogramasList;
        if (data is Map && data.containsKey('data')) {
          cronogramasList = data['data'] as List;
        } else {
          cronogramasList = data as List;
        }
        return cronogramasList.map((c) => Cronograma.fromJson(c)).toList();
      } else if (response.statusCode == 404) {
        return [];
      } else {
        throw Exception('Error al obtener cronogramas eliminados: ${response.statusCode}');
      }
    } catch (e) {
      rethrow;
    }
  ...
  /// Restaurar cronograma eliminado
  Future<bool> restaurarCronograma(int id) async {
    try {
      final headers = await _getHeaders();
      final response = await http.post(
        Uri.parse('$baseUrl/cronogramas/$id/restaurar'),
        headers: headers,
        body: json.encode({}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(errorData['message'] ?? 'Error al restaurar cronograma');
      }
    } catch (e) {
      throw Exception('Error al restaurar cronograma: $e');
    }
  }
  ...
  Future<bool> eliminarPermanentemente(int id) async {
    try {
      final headers = await _getHeaders();
      final response = await http.delete(
        Uri.parse('$baseUrl/cronogramas/$id/forzar'),
        headers: headers,
      );
