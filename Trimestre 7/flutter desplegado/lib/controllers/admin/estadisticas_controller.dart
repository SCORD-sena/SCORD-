...
/// Generar reporte con configuración de tipo
...
Future<List<Partido>> cargarPartidosPorCompetenciaParaEditar(int idCompetencia) async {
  try {    
    ...
    // 🆕 FILTRAR DUPLICADOS POR idPartidos
    final Map<int, Partido> partidosUnicos = {};
    for (var partido in partidosFiltrados) {
      if (!partidosUnicos.containsKey(partido.idPartidos)) {
        partidosUnicos[partido.idPartidos] = partido;
      }
    }
    
    final resultado = partidosUnicos.values.toList();
...
Future<void> cargarEstadisticasParaEditar(int idCompetencia, int idPartido) async {
  ...
  try {
    ...
    if (registro != null) {
      ...
      // 🆕 IMPORTANTE: Cargar los partidos filtrados para el dropdown
      await filtrarPartidosPorCompetencia(idCompetencia);
      ...
      // 🆕 VALIDACIÓN: Verificar que el partido existe en la lista
      final partidoExiste = partidosFiltrados.any((p) => p.idPartidos == idPartido);
      if (!partidoExiste) {
        
        // Intentar agregar el partido manualmente
        final todosLosPartidos = await _cronogramaService.getPartidos();
        final partidoFaltante = todosLosPartidos.firstWhere(
          (p) => p.idPartidos == idPartido,
          orElse: () => throw Exception('No se encontró el partido con ID $idPartido'),
        );
        
        // Agregar a la lista si no está duplicado
        if (!partidosFiltrados.any((p) => p.idPartidos == partidoFaltante.idPartidos)) {
          partidosFiltrados.add(partidoFaltante);
        }
      }
    } else {
