...
/// Widget del menú lateral (Drawer) para Entrenador
class EntrenadorDrawerMenu extends StatelessWidget {
  final Function(String) onItemTap;
  final VoidCallback onLogout;

  const EntrenadorDrawerMenu({
    super.key,
    ...
    return Drawer(
      child: ListView(
        ...
        children: <Widget>[
          ...
          _buildDrawerItem('Estadísticas Jugadores', Icons.bar_chart, '/EstadisticasEntrenador'),
          _buildDrawerItem('Comparar Jugadores', Icons.rule, '/ComparacionJugadoresEntrenador'),
          _buildDrawerItem('Resultados', Icons.scoreboard, '/GestionResultadosEntrenador'),
          _buildDrawerItem('Competencias', Icons.emoji_events, '/CompetenciasEntrenador'),
          const Divider(),
          ListTile(
...
/// Widget de cabecera con bienvenida y foto de perfil del Entrenador
class EntrenadorWelcomeHeader extends StatelessWidget {
  final Persona? entrenadorData;
  final String? categoria;

  const EntrenadorWelcomeHeader({
    super.key,
    required this.entrenadorData,
    this.categoria,
  });
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          Text(
            'Bienvenido(a), ${entrenadorData?.nombreCorto ?? "Entrenador"}',
...
/// Card con información del entrenador
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            _buildInfoRow('Nombre:', entrenadorData?.nombreCompleto ?? 'No disponible'),
            const Divider(),
            _buildInfoRow('Edad:', entrenadorData?.edad?.toString() ?? 'No disponible'),
            const Divider(),
            _buildInfoRow('Género:', entrenadorData?.genero ?? 'No disponible'),
            const Divider(),
            _buildInfoRow(
...
/// Carrusel de información institucional (reutilizado del admin)
