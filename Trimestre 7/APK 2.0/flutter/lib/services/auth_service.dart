...
try {
  ...
  if (tokenFcm != null) {
    await _enviarTokenFcm(tokenFcm, rolId, token);
  }
} catch (e) {
  // Si falla no interrumpir el login
}
  ...
  Future<void> _enviarTokenFcm(String tokenFcm, int rol, String authToken) async {
  try {
    // Dependiendo del rol, guarda en entrenadores o jugadores
    String endpoint = rol == 2 ? '/entrenadores/token-fcm' : '/jugadores/token-fcm';
